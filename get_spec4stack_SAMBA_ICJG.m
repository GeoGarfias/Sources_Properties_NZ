...
for j=list_event;
    ...
    % Work in the folder of each event
    ...
    if exist(['mname_' cluster.name(1,:) '_nsec' num2str(nsec) '_2_egf_SN9.mat']) == 0 % 2 because output from second code
        ...
        if go_ahead == 1
            j = j_current
            all_mname_list(j,:)
            % reload cluster to be sure you have right one.
            ...
            for PS = 1:2
                ...
                ratio_list_name = ['SAMBA_*' PS_letter(PS)]; % SAMBA for samba - written out at end of previous code. 
            ...
            ratio_list_name_out = ['SAMBA_*_*_*_*_*'];
